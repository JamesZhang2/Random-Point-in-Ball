...
(For video: Recall that $\beta_n = c_n \beta_{n - 1}$. If we go from $n-1$ dimensions to $n$ dimensions, $\kappa$ increases by a factor of 2, whereas $\beta$ is multiplied by a factor of $c_n$. Therefore, the ratio between $\beta$ and $\kappa$ gets multiplied by a factor of $\frac{c_n}{2}$ each time we raise a dimension.)
