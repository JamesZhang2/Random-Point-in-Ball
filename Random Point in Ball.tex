...
Let $\beta_n$ be the volume of the $n$-dimensional unit ball. Let $\kappa_n$ be the volume of the $n$-dimensional box that we sample from, where $\kappa$ stands for ``cube''. Since each component is randomly selected in $[-1, 1]$, the box has side length 2, so its $n$-dimensional volume is $2^n$.
