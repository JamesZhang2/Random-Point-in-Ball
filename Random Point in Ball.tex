...
We have shown that $Y_1$ is a standard normal random variable. This is the same for each component of $Y = QX$. A quick calculation of covariance shows that the components of $Y$ are independent, so $Y$ is still a standard multivariate normal. Thus $X$ is rotationally invariant.
